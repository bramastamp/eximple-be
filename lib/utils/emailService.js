...
const sendOtpEmail = async (email, otpCode, purpose = 'email_verification') => {
  try {
    ...
    if (!transporter) {
      console.error('[Email Service] Error: Email service not configured. Check EMAIL_HOST, EMAIL_USER, and EMAIL_PASS in .env');
    ...
    try {
      ...
      if (errorMsg.includes('Invalid login') || errorMsg.includes('BadCredentials')) {
        console.error('[Email Service] Fix: Use Gmail App Password (not regular password). See: https://myaccount.google.com/apppasswords');
      }
    ...
    if (errorMsg.includes('Invalid login') || errorMsg.includes('BadCredentials')) {
      console.error('[Email Service] Fix: Use Gmail App Password. Enable 2-Step Verification and create App Password at: https://myaccount.google.com/apppasswords');
    }
    return { 
      success: false, 
      error: errorMsg
