...
class ProfileController {
  ...
  static async updateProfile(req, res) {
    try {
      const userId = req.user.id;
      const { full_name, gender, grade_level_id, class_id, bio, avatar_url, subject_ids } = req.body;
      ...
      if (subject_ids !== undefined) {
        ...
        } else if (subject_ids !== null && (!Array.isArray(subject_ids) || subject_ids.length === 0)) {
          return res.status(400).json({
            success: false,
            errors: [{
              field: 'subject_ids',
              message: 'Subject IDs must be an array with at least one subject',
        ...
        await UserSubject.setUserSubjects(userId, subject_ids || []);
        userSubjects = await UserSubject.findByUserId(userId);
      }
      ...
      res.status(200).json({
        ...
        data: {
          profile,
          ...(userSubjects !== null && { subjects: userSubjects })
        }
      });
    } catch (error) {
