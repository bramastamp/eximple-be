...
class AuthController {
  static async register(req, res) {
    try {
      ...
      const otpCheck = await OtpCode.checkOtpWithoutMarking(email, otpCodeString, 'email_verification');
      if (!otpCheck.valid) {
        return res.status(400).json({
          success: false,
          errors: [{ field: 'otp_code', message: otpCheck.message, code: 'INVALID_OTP' }]
        });
      }
