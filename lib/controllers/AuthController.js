      ...
      if (existingUser && !existingUser.is_active) {
        ...
        res.status(200).json({
          ...
          data: {
            ...
            ...(process.env.NODE_ENV === 'development' && !emailResult.success && { email_error: emailResult.error })
          }
        });
        return;
      }
    ...
    try {
      ...
      if (!emailResult.success) {
        ...
        return res.status(500).json({
          success: false,
          message: 'OTP created but failed to send email. Please check email configuration.',
  ...
  static async verifyEmail(req, res) {
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        const debugOtp = require('../utils/debugOtp');
        await debugOtp(email, otpCodeString);
      }
