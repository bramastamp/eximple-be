...
const validateCompleteProfile = (req, res, next) => {
  ...
  if (subject_ids !== undefined) {
    if (!Array.isArray(subject_ids)) {
      errors.push({ field: 'subject_ids', message: 'Subject IDs must be an array', code: 'INVALID_SUBJECT_IDS_FORMAT' });
    } else if (subject_ids.length === 0) {
      errors.push({ field: 'subject_ids', message: 'At least one subject must be selected', code: 'SUBJECT_IDS_EMPTY' });
    } else {
      const invalidIds = subject_ids.filter(id => isNaN(parseInt(id)));
      if (invalidIds.length > 0) {
        errors.push({ field: 'subject_ids', message: 'All subject IDs must be numbers', code: 'INVALID_SUBJECT_ID_FORMAT' });
      }
    }
  }
